var MsgUtil = {
	...
	show:function(notice,callback,time){
		...
		{
			var stylehtml = '<style>';	
			stylehtml += '#mpopup_wrap{ background:rgba(0, 0, 0, 0.35); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr=#55000000, endColorstr=#55000000);';
			stylehtml += ' -ms-filter: "progid:DXImageTransform.Microsoft.gradient(startColorstr=#55000000, endColorstr=#55000000)";}';
			stylehtml += '#mpopup h2, #mpopup h2 .close, .mpop_box .button, .mpop_box .button_fd, .loginlist li span, .pop_login li.cwts em{ background:url(http://passport.umiwi.com/static/img/layer_sprite.jpg) no-repeat;} ';
			...
			stylehtml += '#mpopup h2{ background-position:left top; background-repeat:repeat-x; height:33px; line-height:33px; border:1px solid #475529; border-bottom:1px solid #71a205; padding:0; padding-left:12px; color:#000; font-size:12px; font-weight:normal; overflow:hidden; width:548px;}';
			...
			stylehtml += '#mpopup .mpop_line{ background:url(http://passport.umiwi.com/static/img/layer_btbg.jpg) no-repeat bottom center; clear:both; height:12px; margin-bottom:30px;}';
			stylehtml += '#mpopup .mpop_box{ width:560px; background:#fff; padding:30px 0; border:1px solid #666; border-top:0;text-align:center; }';
			...
			var bghtml= '<div style="position: absolute; left: 0px; top: 0px; width: 0px; height: 0px; display: block;" id="mpopup_wrap"></div>';
			var msghtml = stylehtml+bghtml;

			msghtml +='<div id="mpopup">';
			msghtml +='<h2><a href="javascript:void(0);" onclick="MsgUtil.cancel();return false;" class="close"></a>提示</h2>';
