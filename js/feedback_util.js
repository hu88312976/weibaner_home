var FeedbackUtil = {
	feed:function(feedbackurl) {
		var stylehtml = '<style>';
		stylehtml += '#feedback_popup_wrap{z-index:100199; background:rgba(0, 0, 0, 0.6); filter:progid:DXImageTransform.Microsoft.gradient(startColorstr=#99000000, endColorstr=#99000000);';
		stylehtml += ' -ms-filter: "progid:DXImageTransform.Microsoft.gradient(startColorstr=#99000000, endColorstr=#99000000)";}';
		stylehtml += '.pop_box .button_fd, .loginlist li span, .pop_login li.cwts em{ background:url(http://passport.umiwi.com/static/img/layer_sprite.jpg) no-repeat;} ';
		stylehtml += '#feedback_popup { clear:both; left:50%; margin-left:0px; position:absolute; top:300px; width:auto; z-index:100299;}';
		stylehtml += '#feedback_popup h2{ background:#2b9617; height:40px; line-height:40px; padding-left:15px; color:#fff; font-size:16px; font-weight:normal; overflow:hidden; width:545px; position:relative;}';
		stylehtml += '#feedback_popup h2 .close { background:url(http://i3.umivi.net/u/2014/public/images/public_ico.png) no-repeat 1px -450px; cursor:pointer; width:15px; height:15px; top:11px; right:11px; position:absolute; z-index:10; overflow:hidden;}';
		stylehtml += '#feedback_popup .pop_line{ background:url(http://passport.umiwi.com/static/img/layer_btbg.jpg) no-repeat bottom center; clear:both; height:12px; margin-bottom:30px;}';
		stylehtml += '#feedback_popup .pop_box{ width:560px; background:#fff; padding:30px 0; border-top:0;}';
		...
		stylehtml += '#feedback_popup .pop_box .button{ background:#ff7800; color:#fff; text-align:center; font-size:16px; -moz-border-radius:3px; border-radius:3px; float:left; width:128px; height:36px; cursor:pointer; border:0; line-height:36px;}';
		stylehtml += '#feedback_popup .pop_box .button:hover{ color:#fff !important; text-decoration:none;}';
		...
		stylehtml += 'DD_belatedPNG.fix("#feedback_popup h2 .close")';
		stylehtml += '</script>';
		stylehtml += '<![endif]-->';
		...
		html +='<!-- 登录框s -->';
			html +='<ul class="pop_login" style="padding-left:10px;">';
				...
				html +='<li style="height:200px;width:100%"><span class="l">意见建议：</span>';
					html +='<span class="r" style="width:400px;height:100%">';
						html +='<span class="input" style="width:400px;height:180px">';
							html +='<textarea tabindex="2" onclick="FeedbackUtil.onfocus();" style="width:400px;height:180px;color:#999999;font-size:14px;line-height:18px;" rows="" cols="" id="fb_content" name="content" >请在这里输入您对优米网的意见和建议。视频播放问题请留下QQ号或者手机号便于联系。</textarea>';
						html +='</span>';
					html +='</span>';
				html +='</li>';
				...
				html +='<li class="zc">';
					...
					html +='<input type="button" onclick="FeedbackUtil.send(\''+feedbackurl+'\')" value="提交" class="button" tabindex="7">';
				html +='</li>';
			html +='</ul>';
			html +='</div>';
			html +='<!-- 登录框e -->';
	...
	send:function(feedbackurl) {
		...
		$.getScript('http://v.youmi.cn/feedback/add?app=web&t='+Date.parse(new Date())+'&email='+encodeURIComponent(email)+'&content='+encodeURIComponent(content)+'&url='+encodeURIComponent(url));
	...
	onfocus:function(){
		if($("#fb_content").val() == '请在这里输入您对优米网的意见和建议。'){
			$("#fb_content").val('')
		}
